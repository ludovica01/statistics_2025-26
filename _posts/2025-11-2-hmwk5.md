...
Let $S_{n-1}$ be the previous sum of squared differences. The new sum $S_n$ can be related to $S_{n-1}$ using the following update, which incorporates the previous mean (Xₙ-1) and the new mean (Xₙ) [4, 5]:
<br>
<code>Sₙ = S(n-1) + (Xₙ - Mean(n-1)) * (Xₙ - Mean(n))</code>
<br>

While the term $(X_n - \bar{X}_n)$ can be complex to calculate recursively, we substitute the mean update formula ($Xₙ = \bar{X}_{n-1} + \delta_n / n$, where $\delta_n = Xₙ - Xₙ-1 is the error) [5]:
<br>
