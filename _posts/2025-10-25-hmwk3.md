...
For recursive calculation, we focus on how $S_n$ relates to $S_{n-1}$ and the new data point $X_n$ [4]. This derivation often follows **Welford’s method** to maintain numerical stability [4].

Let $S_{n-1}$ be the previous sum of squared differences [4]. The new sum $S_n$ can be related to $S_{n-1}$ using the following update, which incorporates the previous mean ($\bar{X}_{n-1}$) and the new mean ($\bar{X}_n$) [4]:
<br>
...
def calculate_online_statistics(data_stream):
    ...
    for x in data_stream: 
        ...
        if n == 1: 
            ...
            # 1. Update Mean (Recurrence Formula: X_bar_n = X_bar_(n-1) + (X_n - X_bar_(n-1)) / n) [7]
            delta = x - mean 
            mean += delta / n 
...
# Example usage with random data [7]
